...
class PBE():
    ...
    def inputPT(self):
        ...
        print("Alice's plaintext: %s" % plaintext)
        return plaintext
    ...
    def PBE_Decrypt(self, alicePW):
        # password input
        alicePassword = self.inputDecryptPW(alicePW)
    ...
    def inputDecryptPW(self, alicePW):
        count = 0
        while True:
            password = input("Input Password: ").encode('utf-8')
            if password != alicePW:
                ...
                if(count == 3):
                    sys.exit()
                print(count, "/3 times left. WrongPassWord! Input Correct PW.")
            if password == alicePW:
                print("Alice's password: %s" % password)
                return password
...
def main():
    pbe = PBE()
    alice_PW = pbe.PBE_Encrypt()
    pbe.PBE_Decrypt(alice_PW)
