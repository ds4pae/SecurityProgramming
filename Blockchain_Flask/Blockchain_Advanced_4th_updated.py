...
def addPublicKeyOnBlockchain(my_blockchain, userName):
    ...
    #publicKey = str(publicKey)  ## RSA 키 암복호화 과정, base64 encoding/decoding 방식 필요
    publicKeyBase64 = base64.b64encode(publicKey)  ## 수정됨....
    my_blockchain.mine_block(userName, publicKeyBase64.decode())  # 수정됨....
...
if __name__ == '__main__':
    ...
    publicKey = base64.b64decode(yourPublicKey)
    print(f"publicKeyDecoded : {publicKey}")
    ciphertext = rsaEncrypt(plaintext, publicKey)
    print(f"ciphertext : {ciphertext}")

    with open("./PrivateKey/Carol_privateKey.pem", "rb") as f:
        privateKey = f.read()
        f.close()
    decrypted = rsaDecrypt(ciphertext, privateKey)
